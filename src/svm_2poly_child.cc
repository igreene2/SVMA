...
void DefineTwoPolyChild(Ila& m) {
    ...
    // child_end: take the sign of the result in final_sum and set the score, set child to invalid
    {
        ...
        auto instr = child.NewInstr("child_end_poly2");
        ...
        instr.SetUpdate(m.state("output"), Ite(Greatest(sub_th, BvConst(0, 32)), BvConst(1, 1), BvConst(0, 1)));
